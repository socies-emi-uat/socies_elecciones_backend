...
public class LoadTestServiceTest {
    ...
    void testLoadTestConfigValidation() {
        // Test valid config
        LoadTestConfigDTO validConfig = new LoadTestConfigDTO(
            "LOAD", "/api/test", 5, 50, 30, "Test config"
        );
        assertTrue(validConfig.isValid());

        // Test invalid config - missing testType
        LoadTestConfigDTO invalidConfig1 = new LoadTestConfigDTO(
            null, "/api/test", 5, 50, 30, "Test config"
        );
        assertFalse(invalidConfig1.isValid());

        // Test invalid config - invalid testType
        LoadTestConfigDTO invalidConfig2 = new LoadTestConfigDTO(
            "INVALID", "/api/test", 5, 50, 30, "Test config"
        );
        assertFalse(invalidConfig2.isValid());

        // Test invalid config - zero concurrent users
        LoadTestConfigDTO invalidConfig3 = new LoadTestConfigDTO(
            "LOAD", "/api/test", 0, 50, 30, "Test config"
        );
    ...
    void testLoadTestResultCreation() {
        ...
        LoadTestResult result = new LoadTestResult(
            "LOAD", "/api/test", 5, 100, 95, 5, 
            250.0, 100.0, 500.0, 3.8,
    ...
    void testSuccessRateCalculation() {
        ...
        LoadTestResult perfectResult = new LoadTestResult(
            "LOAD", "/api/test", 5, 100, 100, 0, 
            250.0, 100.0, 500.0, 3.8,
            LocalDateTime.now().minusMinutes(5),
            LocalDateTime.now()
        );
        ...
        // Test 0% success rate
        LoadTestResult failedResult = new LoadTestResult(
            "LOAD", "/api/test", 5, 100, 0, 100, 
            250.0, 100.0, 500.0, 0.0,
            LocalDateTime.now().minusMinutes(5),
            LocalDateTime.now()
        );
        ...
        LoadTestResult partialResult = new LoadTestResult(
            "LOAD", "/api/test", 5, 100, 75, 25, 
            250.0, 100.0, 500.0, 2.5,
            LocalDateTime.now().minusMinutes(5),
            LocalDateTime.now()
        );
