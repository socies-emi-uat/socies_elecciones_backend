...
public class VVotoController {
    ...
    public ResponseEntity<ResponseWrapper<List<UVotoDTO>>> findAll() {
        ...
        ResponseWrapper<List<UVotoDTO>> votos = new ResponseWrapper<>(true, "Todos los votos obtenidos", votoDTOS);
    ...
    @PostMapping("/votar")
    public ResponseEntity<ResponseWrapper<UVotoDTO>> votar(@RequestBody UVotoCreateDTO uVotoCreateDTO) {
        UVotoDTO uVotoDTO = votoService.save(uVotoCreateDTO);
        ResponseWrapper<UVotoDTO> response = new ResponseWrapper<>(true, "Voto realizado sin ningun problema.", uVotoDTO);
