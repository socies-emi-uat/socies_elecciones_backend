...
public class ProvinciaController {
    ...
    public ResponseEntity<ResponseWrapper<List<ProvinciaDTO>>> findAll() {
        ...
        ResponseWrapper<List<ProvinciaDTO>> provinciasResponse = new ResponseWrapper<>(true, "Todas las provincias obtenidos.", provincias);
    ...
    public ResponseEntity<ResponseWrapper<ProvinciaDTO>> findById(@PathVariable Long id_provincia) {
        ...
        ResponseWrapper<ProvinciaDTO> provinciaDTOResponseWrapper = new ResponseWrapper<>(true, "Provincia obtenido.", provinciaDTO);
        return new ResponseEntity<>(provinciaDTOResponseWrapper, HttpStatus.OK);
    }

    @PostMapping
    public ResponseEntity<ResponseWrapper<ProvinciaDTO>> create(@Valid @RequestBody ProvinciaCreateDTO provinciaDTO) {
        ProvinciaDTO provinciaDTO1 = provinciaService.create(provinciaDTO);
        ResponseWrapper<ProvinciaDTO> provinciaDTOResponseWrapper = new ResponseWrapper<>(true, "Provincia creado con exito.", provinciaDTO1);
        return new ResponseEntity<>(provinciaDTOResponseWrapper, HttpStatus.CREATED);
    }

    @PutMapping("/id_provincia")
    public ResponseEntity<ResponseWrapper<ProvinciaDTO>> update(@Valid @RequestBody ProvinciaUpdateDTO provinciaUpdateDTO, @PathVariable Long id_provincia) {
        ProvinciaDTO provinciaDTO = provinciaService.update(id_provincia, provinciaUpdateDTO);
        ResponseWrapper<ProvinciaDTO> provinciaDTOResponseWrapper = new ResponseWrapper<>(true, "Provincia creado con exito.", provinciaDTO);
        return new ResponseEntity<>(provinciaDTOResponseWrapper, HttpStatus.OK);
    }
}
