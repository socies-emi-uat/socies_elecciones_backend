...
public class PartidoController {
    ...
    public ResponseEntity<ResponseWrapper<List<PartidoDTO>>> getAll() {
        ...
        ResponseWrapper<List<PartidoDTO>> procesos =
                new ResponseWrapper<>(
                        true, "Lista de todos los partidos.", partidosDTO);
    ...
    public ResponseEntity<ResponseWrapper<PartidoDTO>> findById(@PathVariable Long id_partido) {
        ...
        ResponseWrapper<PartidoDTO> partido = new ResponseWrapper<>(true, "Partido encontrado.", partidoDTO);
        return new ResponseEntity<>(partido, HttpStatus.OK);
    }

    @PostMapping
    public ResponseEntity<ResponseWrapper<PartidoDTO>> create(@Valid @RequestBody PartidoCreateDTO partidoCreateDTO) {
        PartidoDTO partidoDTO = partidoService.create(partidoCreateDTO);
        ResponseWrapper<PartidoDTO> partido = new ResponseWrapper<>(true, "Partido criado.", partidoDTO);
        return new ResponseEntity<>(partido, HttpStatus.CREATED);
    }

    @PutMapping("/{id_partido}")
    public ResponseEntity<ResponseWrapper<PartidoDTO>> update(@Valid @RequestBody PartidoUpdateDTO partidoUpdateDTO, @PathVariable Long id_partido) {
        PartidoDTO partidoDTO = partidoService.update(id_partido, partidoUpdateDTO);
        ResponseWrapper<PartidoDTO> partido = new ResponseWrapper<>(true, "Partido actualizado correctamente", partidoDTO);
