...
public class ProcesoElectoralController {
    ...
    public ResponseEntity<ResponseWrapper<List<ProcesoElectoralDTO>>> getAll() {
        ...
        ResponseWrapper<List<ProcesoElectoralDTO>> procesos = new ResponseWrapper<>(true, "Lista de todos los procesos electorales.", procesoElectoralDTOS);
    ...
    public ResponseEntity<ResponseWrapper<ProcesoElectoralDTO>> getById(@PathVariable Long id) {
        ...
        ResponseWrapper<ProcesoElectoralDTO> procesos = new ResponseWrapper<>(true, "Prceso electoral obtenido.", procesoElectoralDTO);

        return new ResponseEntity<>(procesos, HttpStatus.OK);
    }

    @PostMapping
    public ResponseEntity<ResponseWrapper<ProcesoElectoralDTO>> crearProcesoElectoral(@Valid @RequestBody ProcesoElectoralCreateDTO procesoElectoralDTO) {
        ProcesoElectoralDTO proceso = procesoElectoralService.create(procesoElectoralDTO);
        ResponseWrapper<ProcesoElectoralDTO> nuevo_proceso = new ResponseWrapper<>(true, "Proceso electoral creada.", proceso);

        return new ResponseEntity<>(nuevo_proceso, HttpStatus.CREATED);
    }

    @PutMapping("/{id_prceso}")
    public ResponseEntity<ResponseWrapper<ProcesoElectoralDTO>> actualizarProcesoElectoral(@Valid @RequestBody ProcesoElectoralUpdateDTO procesoElectoralDTO, @PathVariable Long id) {
        ProcesoElectoralDTO proceso = procesoElectoralService.actualizar(procesoElectoralDTO, id);
        ResponseWrapper<ProcesoElectoralDTO> proceso_actualizado = new ResponseWrapper<>(true, "Proceso electoral creada.", proceso);
