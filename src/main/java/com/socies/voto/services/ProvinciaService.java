...
public class ProvinciaService {
    ...
    public ProvinciaDTO findById(Long id) {
        return provinciaRepository.findById(id).map(ProvinciaDTO::new).orElseThrow(() -> new ResourceNotFoundException("Provincia no encontrado"));
    }

    public ProvinciaDTO create(ProvinciaCreateDTO provinciaDTO) {
        provinciaRepository.findByNombre(provinciaDTO.getNombre()).ifPresent(provincia -> {
    ...
    public ProvinciaDTO update(Long id, ProvinciaUpdateDTO provinciaUpdateDTO) {
        provinciaRepository.findByNombre(provinciaUpdateDTO.getNombre()).ifPresent(provincia -> {
            throw new ResourceNotFoundException("Provincia ya existente");
        });

        Provincia provincia = provinciaRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Provincia no encontrado"));
