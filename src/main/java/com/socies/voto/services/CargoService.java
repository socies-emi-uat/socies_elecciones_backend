...
public class CargoService {
    ...
    public CargoDTO obtenerCargoPorId(Long id) {
        return cargoRepository.findById(id).map(cargo -> new CargoDTO(cargo.getId(), cargo.getNombre(), cargo.getDescripcion())).orElseThrow(() -> new CargoNotFoundException("El cargo no fue encontrado"));
    }

    public CargoDTO CrearCargo(CargoCreateDTO cargoCreateDTO) {
        // ✅ Verificar si ya existe por nombre
        cargoRepository.findByNombre(cargoCreateDTO.getNombre_cargo())
                .ifPresent(c -> { throw new CargoAlreadyExistsException("El cargo ya existe"); });
