...
@Service
@ConditionalOnProperty(name = "loadtest.scheduled.enabled", havingValue = "true", matchIfMissing = false)
public class ScheduledLoadTestService {
    ...
    public void runDailyLoadTests() {
        ...
        for (String endpoint : criticalEndpoints) {
            try {
                LoadTestConfigDTO config = new LoadTestConfigDTO(
                    "LOAD",
                    endpoint,
                ...
                logger.error("Error en prueba de carga para endpoint {}: {}", endpoint, e.getMessage());
            }
        }
    ...
    public void runWeeklyStressTests() {
        ...
        for (String endpoint : criticalEndpoints) {
            try {
                ...
                logger.error("Error en prueba de estrés para endpoint {}: {}", endpoint, e.getMessage());
            }
        }
