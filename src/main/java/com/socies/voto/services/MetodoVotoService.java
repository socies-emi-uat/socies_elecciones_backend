...
public class MetodoVotoService {
    ...
    public MetodoVotoDTO findById(Long id) {
        return metodoVotoRepository.findById(id)
                .map(MetodoVotoDTO::new)
                .orElseThrow(() -> new MetodoVotoNotFoundException("Método de voto no encontrado."));
    }
    public MetodoVotoDTO create(MetodoVotoCreateDTO metodoVotoCreateDTO) {
        metodoVotoRepository.findByNombre(metodoVotoCreateDTO.getNombre())
                .ifPresent(metodoVoto -> {
                    throw new MetodoVotoAlreadyExistsException("El método de voto ya existe.");
                });
    ...
    public MetodoVotoDTO update(Long id, MetodoVotoUpdateDTO metodoVotoUpdateDTO) {
        MetodoVoto metodoVoto = metodoVotoRepository.findById(id)
                .orElseThrow(() -> new MetodoVotoNotFoundException("Método de voto no existe."));
        metodoVotoRepository.findByNombre(metodoVotoUpdateDTO.getNombre())
