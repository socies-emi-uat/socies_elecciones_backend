...
public class LoadTestService {
    ...
    @Autowired private LoadTestResultRepository loadTestResultRepository;
    ...
    public LoadTestResultDTO executeLoadTest(LoadTestConfigDTO config) {
        ...
        logger.info(
                "Prueba de carga completada. Éxito: {}, Fallos: {}",
                successCount.get(),
                failureCount.get());
    ...
    private LoadTestResult calculateMetrics(
    ...
    public List<LoadTestResultDTO> getResultsByDate(LocalDate date) {
        return loadTestResultRepository.findByTestDate(date).stream()
    ...
    public DailyTestSummaryDTO getDailySummary(LocalDate date, String testType) {
        List<LoadTestResult> results =
                loadTestResultRepository.findByTestDateAndTestType(date, testType);
        List<LoadTestResultDTO> resultDTOs =
                results.stream().map(LoadTestResultDTO::new).collect(Collectors.toList());

        return new DailyTestSummaryDTO(date, testType, resultDTOs);
    }

    // Obtener resultados por rango de fechas
    public List<DailyTestSummaryDTO> getResultsByDateRange(
            LocalDate startDate, LocalDate endDate, String testType) {
